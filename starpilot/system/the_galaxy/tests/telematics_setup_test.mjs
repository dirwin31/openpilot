...
assert.equal(ready.calls[0].url, 'https://galaxy.firestar.link/1234567890abcdef/assets/mobile/telematics-worker.js')
assert.equal(ready.calls[0].options.scope, '/1234567890abcdef/assets/mobile/', 'Dedicated worker controls the separate app launch URL')
assert.equal(ready.unregistered, false, 'Keep the dedicated worker and Galaxy notification registrations')
