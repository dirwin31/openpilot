...
def _supervised(task, *, check_parked, abort, monotonic, timeout, poll=0.5):
  """Run a blocking task under the parked and timeout guards, calling `abort` if either fails.

  Returns the task's own exception, if any.
  """
  outcome = []
  ...
  try:
    ...
    abort()
    # Hold the model workflow until the refresh exits.
    worker.join()
    raise
...
def download_saved_models(models, *, catalog, queue, cancel, owns, busy, progress, check_parked, reboot, report,
  ...
  """Download saved models by catalog ID (archives never supply URLs), then reboot.

  Unoffered models are skipped and reported; saved versions are informational since the downloader
  serves the catalog's version. Reboot only after every attempted download is verified.
  """
  check_parked()
  if busy():
