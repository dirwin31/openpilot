...
import { telematicsOffline, saveTelematicsOffline, dismissOfflineBanner, showOfflineBanner, cachedTelematicsURL } from "../telematics-install.js"
...
export const PhonePanel = {
  ...
  computed: {
    offlineDashboardURL() { return cachedTelematicsURL() },
  ...
  methods: {
    setupOffline() { return saveTelematicsOffline() },
    dismissOffline() { dismissOfflineBanner() },
    ...
    <div style="padding: var(--sp-3);">
      ...
      <div v-else-if="!onGalaxyLink || platform === 'insecure'" class="telematics-gate">
        ...
        <ol class="telematics-gate__steps">
          ...
          <li>On the Phone page, use “Make available offline” to save Telematics and add a home-screen shortcut.</li>
        </ol>
      </div>

      <div v-else class="telematics-bluetooth-setup">
        <GxNotice v-if="!telematicsOffline.dismissed" class="telematics-offline-banner" tone="info" icon="bi-download" :title="offlineState.state === 'ready' ? 'Telematics is available offline' : 'Make Telematics available offline'">
          <p v-if="offlineState.state !== 'ready'">Save the dashboard on this phone to use Bluetooth Telematics without internet. Save it while your phone and comma are online.</p>
          <button v-if="offlineState.state !== 'ready'" class="gx-btn" type="button" :disabled="offlineState.state === 'saving'" @click="setupOffline">{{ offlineState.state === 'saving' ? 'Saving…' : offlineState.state === 'error' ? 'Retry saving' : 'Make available offline' }}</button>
          <template v-else>
            <p>Open the saved dashboard, then add that page to your home screen:</p>
            <ol>
              <li>Tap <b>Open saved Telematics</b> below.</li>
              <li>On that page, open Chrome’s <b>⋮ → Add to Home screen → Create shortcut</b>.</li>
              <li>Name it <b>Telematics</b> and tap <b>Add</b>.</li>
            </ol>
            <a class="gx-btn" :href="offlineDashboardURL">Open saved Telematics</a>
          </template>
          <p v-if="offlineState.state === 'error'" role="alert">{{ offlineState.message }}</p>
          <button class="gx-btn gx-btn--text" type="button" @click="dismissOffline">Dismiss</button>
        </GxNotice>
        <button v-else class="gx-btn gx-btn--text" type="button" @click="showOffline">Offline Telematics help</button>
        ...
        <ol class="telematics-pair-steps">
          ...
          <li>
            ...
            <p>Connect, then reload near your comma to check it reconnects. Use the offline banner above to save the dashboard and add a home-screen shortcut.</p>
