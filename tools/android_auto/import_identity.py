"""Import the phone TLS identity from the verified Android Auto 17.6 APK.

Development-time tool, run on a computer (not the comma); requires jadx, Android
SDK apksigner, and cryptography. No APK, decompiled source, certificate, or key
is distributed with this tool, and its output must never be committed.
...
Copied from yummydirtx/openpilot ``tools/android_auto/import_identity.py`` (MIT),
pinned at 672a16f6183567c0ada53654f8527d97e1a483fa; only the default output path
and temporary-directory prefix differ. See docs/how-to/wireless-android-auto.md.
"""
...
def main():
  ...
  parser.add_argument("--output", type=Path, default=Path(".cache/android_auto/identity"), help="New private output directory")
  ...
  with tempfile.TemporaryDirectory(prefix="android-auto-jadx-") as temporary:
    sources = []
    for name in ("jcf", "jch"):
